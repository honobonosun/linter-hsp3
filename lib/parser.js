...
const newline = (str1, str2) => (str1 === "\r" && str2 === "\n") || str1 === "\n";
const regexps = (regexp, text) => {
    let i = 0;
    return regexp.map(v => v.exec(text[i++]));
};
const g = new potage.Generator();
/**
 * target引数に渡されたデータから行単位に区切り、resultプロパティにstring[]型で格納します。
 * @param cnt 何行取得するか
 */
const line = (cnt = 0) => g.satisfy((t, p) => {
    if (p >= t.length)
        return undefined;
    let i = 0;
    let offset = p;
...
const macro_stack_is_not_free = g.satisfy((t, p) => {
    ...
    return {
        next: l.location[0],
        far: l.location[1],
        result: new message("スタックが空になっていないマクロタグがあります", "error", {
            file: r[0][1],
            word: r[1][1]
        })
    };
...
const syntaxError = g.satisfy((t, p) => {
    ...
    if (!l.success)
        return undefined;
    const r = regexps([/(.+)\((\d+)\) : error (\d+) : (.+) \((\d+)行目\)/i, /\s*-->\s*(.+)/i], l.result);
    if (r[0] && r[1])
        return {
            next: l.location[0],
            far: l.location[1],
...
const criticalError = g.satisfy((t, p) => {
    ...
    if (!l.success)
        return undefined;
    const next = l.location[0], far = l.location[1];
    let r;
...
export const parser = (source) => g.map(g.many(g.choice(variable_is_not_initialized_exist, macro_stack_is_not_free, include_file_is_not_found, nonDefined, syntaxError, criticalError, any)), r => r.result.many.filter((v) => v.choice).map((v) => v.choice))(Array.from(source), 0);
