 ...
 */
const escapeBackSlash = (source: string[]) =>
  source.map((v) => v.replace(/\\/g, "\\\\"));

/**
 * hspc.v1系向けに、ダブルクォーテーションを削除します。
 * @param source hspcへの引数
 */
const removeQuotation = (source: string[]) => [
...
export const exec = (
  ...
  new Promise((resolve, reject) => {
    if (get.compiler.useEscape()) args = escapeBackSlash(args);
    if (
      (cmd.toLowerCase() !== "wine" ||
        cmd.toLowerCase() !== "winepath" ||
        cmd.toLowerCase() !== "wslpath") &&
      get.compiler.useKillQuiotations()
    )
      args = removeQuotation(args);
    if (app.inDevMode()) console.log("linter-hsp3", "exec", cmd, args);
    ...
    proc.on("close", () => {
      if (app.inDevMode()) console.log("linter-hsp3", "proc" ,proc);
      if (decode) return resolve(iconv.decode(cache, decode));
      else return resolve(cache.toString("utf8"));
    });
  });
 ...
 */
export const cwd = (file: string): string | undefined => {
  const mode = get.compiler.curdir();
  if (mode === "file") return path.dirname(file);
