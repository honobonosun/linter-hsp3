"use babel";
/**
 * Generatorクラスで作成されたパーサ関数がパースした結果、返される型です。
 *
 * @property success パースの成否を表します。
 * @property location: 次の三つ要素を持っています。左から、次に読む位置、読み始めた位置、読み終えた位置を表します。
 * @property result パーサ関数がパースした結果が格納されています。型はanyです。
 * @property log ロギングメソッドの結果が格納されています。
 */
...
/**
 * Generatorクラスのパーサジェネレーターメソッドが返すパーサ関数です。
 *
 * @param target パースするデータ
 * @param position パースを開始する位置
 */
export interface IParser<T> {
  (target: T, position: number): IResult;
}
...
export class Generator<T> {
  ...
  satisfy(
    fn: (
      target: T,
      position: number
    ) => { next: number; far: number; result: any } | undefined
  ): IParser<T> {
    return function(target, position) {
      const value = fn(target, position);
      let location: IResult["location"] = [position, position, position];
      let result: IResult["result"] = undefined;
      if (value) {
   ...
   */
  private static logging<T>(
    parser: IParser<T>,
    fn: (result: IResult) => string | undefined
  ): IParser<T> {
    return function(target, position) {
      const parsed = parser(target, position);
      const text = fn(parsed);
      if (text) parsed.log.push(text);
      return parsed;
    };
  }
  ...
  logging(
    ...
    return Generator.logging<T>(parser, fn);
  }
   ...
   */
  desc(parser: IParser<T>, fn: (result: IResult) => string): IParser<T> {
    return Generator.logging<T>(parser, r => (r.success ? undefined : fn(r)));
  }
   ...
   */
  seq(...parsers: IParser<T>[]): IParser<T> {
    return function(target, position) {
      const readPosition = position;
      let farPosition = position;
      let success = true;
      let result = { seq: [] as any };
      let log: string[] = [];
      ...
      const location: IResult["location"] = [
        position,
        readPosition,
        farPosition
      ];
   ...
   * @param parsers 成功を望むパーサ関数たち
   */
  choice(...parsers: IParser<T>[]): IParser<T> {
    return function(target, position) {
      let log: string[] = [];
      for (const parser of parsers) {
        const parsed = parser(target, position);
   ...
   */
  many(parser: IParser<T>, fn?: (parsed: IResult) => boolean): IParser<T> {
    return function(target, position) {
      const readPosition = position;
      let farPosition = position;
      let result = { many: [] as any };
      let log: string[] = [];
      for (;;) {
        const parsed = parser(target, position);
        log = log.concat(parsed.log);
   ...
   */
  optional(parser: IParser<T>): IParser<T> {
    return function(target, position) {
   ...
   */
  map(parser: IParser<T>, fn: (parsed: IResult) => any): IParser<T> {
    return function(target, position) {
      const parsed = parser(target, position);
      if (parsed.success) {
        return {
   ...
   */
  lazy(fn: () => IParser<T>): IParser<T> {
    let parser: IParser<T>;
    return function(target, position) {
      if (!parser) parser = fn();
